...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        while (stopIterator.hasNext()){
            ...
            if (Geometry.rectangleContainsPoint(northWest, southEast, nextStop.getLocn())){
                ...
                m.setPosition(Geometry.gpFromLL(nextStop.getLocn()));
                m.setTitle(nextStop.getNumber() + " " + nextStop.getName() + getRouteNo(nextStop));
                //m.setSubDescription();
                m.setRelatedObject(nextStop);

                setMarker(nextStop, m);
    ...
    /**
     * Get route numbers at stop as one string.
     */
